...
const EditarTrabajo = () => {
    ...
    const {alerta, mostrarAlerta, submitTrabajo, trabajo, obtenerTrabajo} = useTrabajos()

	
	//obtener valores del trabajo
	const { cliente, ubicacion, hectareas, agroquimico, fecha, _id} = trabajo;
	
	const [clienteEdit, setCliente] = useState(cliente)
    const [ubicacionEdit, setUbicacion] = useState(ubicacion)
    const [hectareasEdit, setHectareas] = useState(hectareas)
    const [agroquimicoEdit, setAgroquimico] = useState(agroquimico)
    const [fechaEdit, setFecha] = useState(fecha)
	...
	const handleSubmit = async e =>{
        ...
        await submitTrabajo({id,clienteEdit, ubicacionEdit, hectareasEdit, agroquimicoEdit, fechaEdit })
	...
	return (
		<div className="md:flex md:justify-center items-start md:gap-40">
			<form className="md:w-1/2 transform overflow-hidden rounded-md bg-white p-4 text-left  shadow-xl transition-all" onSubmit={handleSubmit}>
				...
				<div className="mt-4 flex justify-center">
					<button type='submit' className='shadow-md bg-green-600 p-3 md:p-2 font-bold text-white text-center rounded-md hover:bg-green-700 transition-all text-xl md:text-base'>{id ? "Actualizar": "Confirmar"}
					</button>
				</div>
			</form>
			<Link to={`/trabajos/${params.id}`} className="mt-4 bg-green-600 p-2 text-white rounded-md font-bold flex justify-center hover:bg-green-700 transition-all  text-center">Volver</Link>
		</div>
	)
}
